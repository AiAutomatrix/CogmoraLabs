...
// Image from next/image is not used in the provided snippet, so removing the import.
// If it's needed later for other parts of this component (e.g. token headers), it can be re-added.
  ...
  const fetchDataForView = useCallback(async (view: DexScreenerViewType) => {
    ...
    try {
      ...
      if (result && !Array.isArray(result)) {
        setData([result as any]); // Cast to any if single object, then wrap
      } else {
        setData(result || []); // Handle null/undefined result
      }
    } catch (err) {
