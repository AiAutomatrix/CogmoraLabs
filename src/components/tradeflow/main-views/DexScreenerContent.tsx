...
const DexScreenerContent: React.FC = () => {
  ...
  const renderTable = () => {
              ...
              <TableRow key={`${item.tokenAddress}-${item.chainId}-${index}-${selectedView}`}>
                <TableCell>
                  <Avatar className="h-8 w-8">
                    <AvatarImage src={item.icon ?? `https://placehold.co/32x32.png`} alt={item.name || 'Token icon'} />
                    <AvatarFallback>{(item.name || 'NA').substring(0, 2).toUpperCase()}</AvatarFallback>
                  </Avatar>
                </TableCell>
    ...
    if (selectedView === 'tokenOrders') {
      ...
      if (items.length === 0) return <div className="flex items-center justify-center h-full"><p className="text-muted-foreground">No orders found for this token.</p></div>;
      return (
        <Table>
          <TableCaption>Paid orders for the specified token.</TableCaption>
          <TableHeader>
            <TableRow>
          ...
          <TableBody>
            {items.map((item, index) => (
              <TableRow key={`${item.type}-${item.paymentTimestamp}-${index}`}>
                ...
                <TableCell className="text-right">{formatDateFromTimestamp(item.paymentTimestamp)}</TableCell>
              </TableRow>
            ))}
          </TableBody>
        </Table>
    ...
    if (['pairDetailsByPairAddress', 'searchPairs', 'tokenPairPools', 'pairsByTokenAddresses'].includes(selectedView)) {
      ...
      if (pairs.length === 0) return <div className="flex items-center justify-center h-full"><p className="text-muted-foreground">No pair data available for this view.</p></div>;
            ...
            {pairs.map((pair, index) => (
              <TableRow key={`${pair.pairAddress}-${pair.chainId}-${index}`}>
                ...
                <TableCell className="font-medium max-w-[180px] min-w-0">
                  <Tooltip delayDuration={100}>
                    <TooltipTrigger asChild>
                      <div className="truncate" title={`${pair.baseToken?.name || pair.baseToken?.symbol} / ${pair.quoteToken?.name || pair.quoteToken?.symbol}`}>
        ...
        >
          {viewOptions.map(opt => (
            <div key={opt.value} className="flex items-center">
              <RadioGroupItem value={opt.value} id={`dex_view_${opt.value}`} className="peer sr-only" />
              <Label 
                htmlFor={`dex_view_${opt.value}`}
          ...
          <DialogContent className="sm:max-w-[600px] max-h-[90vh] flex flex-col">
            <DialogHeader>
              ...
              <DialogDescription>
                ...
                <Button variant="ghost" size="icon" className="h-5 w-5 ml-1" onClick={() => selectedPairForDialog.pairAddress && handleCopyAddress(selectedPairForDialog.pairAddress)}><Copy className="h-3 w-3"/></Button>
                <br/>
