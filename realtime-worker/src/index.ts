...
async function processPriceUpdate(symbol: string, price: number) {
    ...
    try {
        ...
        const positionsSnapshot = await db.collectionGroup('openPositions').where('symbol', '==', symbol).get();
        ...
        // 2️⃣ Evaluate triggers in memory
        const triggersSnapshot = await db.collectionGroup('tradeTriggers').where('symbol', '==', symbol).get();
        ...
        // ✅ NOTE: No watchlist currentPrice updates
